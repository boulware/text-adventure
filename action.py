...
def match_regex(action, action_string):
	bad_syntax = False

	match = action.regex.fullmatch(action_string)
	if not match:
		print("Invalid syntax. Try: \'{}\'".format(action.syntax))
		print("Regex pattern: \'{}\'".format(action.regex.pattern))
		print("Action string: \'{}\'".format(action_string))
		bad_syntax = True

	return bad_syntax, match

def do(action, action_string, time, actions_group):
	bad_syntax, match = match_regex(action, action_string)
	...
	if not bad_syntax:
		...
		event_string = sub_roles(action, roles_dict)

		if action.is_stative:
			print("{}.".format(event_string))
			return State(action.lemma, roles_dict, actions_group=actions_group) # (TODO) Hacky solution using action.lemma to have cross-support with file loading
		else:
			print("{} at {} o'clock.".format(event_string, time))
			return m_event.Event(time, action, roles_dict)

	return None

def sub_roles(action, roles_dict):
	subbed_string = action.syntax

	subbed_string = re.sub('\*', action.inflection, subbed_string)
...
def parse_syntax(action, syntax):
	parameters = []
	roles = []
	...
	for word in syntax.split():
		if word == "*":
			pattern += '\s+(?P<VERB>{})'.format(action.lemma) # (TODO) Change the syntax to allow direct writing of the action form in instead of '*' so it reads more naturally.
			parameters.append(Parameter(ParameterType.Command, "action"))
		elif word.isupper():
